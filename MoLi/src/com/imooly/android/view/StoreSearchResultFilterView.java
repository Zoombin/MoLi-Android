...
public class StoreSearchResultFilterView extends LinearLayout {
				...
				if (type == FilterType.SORT) {
					...
					if (type == FilterType.CATEGRORY) {
						...
						if (subList != null && subList.size() > 0) {
							...
							//没有子分类
							SubClassifyEntity subEntity = new SubClassifyEntity();
							subEntity.setClassifyid(fyEty.getClassifyid());
							subEntity.setClassifyname(fyEty.getClassifyname());
							callBack.onSelect(type, subEntity, position);
						}
					}
	...
	public void show(FilterType type, List<?> list, int PSelect) {
		...
		if (type == FilterType.SORT) {
			...
			android.widget.LinearLayout.LayoutParams params = new android.widget.LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, 450);
			level1_list.setLayoutParams(params);
