...
public class Utils {
	...
	public static File getCacheFile(Context context) {
		...
		if (sdacrdExist()) {
			...
			if (!Environment.getExternalStorageState().equals(
				...
				dirPath = String.format("/%s/%s/longjx/%s/", "data/data",
						context.getPackageName(), QMD5.encryptMD5("cache"));
			}
			cacheFile = new File(dirPath);
			if (!cacheFile.exists())
				FileUtils.getInstance(context).createDirectory(dirPath);
		}
		return cacheFile;
	}
	 ...
	 * @param edittext
	 */
	public static void setEditTextHint(EditText edittext) {
		// 新建一个可以添加属性的文本对象
	...
	public static boolean checkIsInstallWeiXin(Context context) {
		List<PackageInfo> packs = context.getPackageManager().getInstalledPackages(0);
		for (int i = 0; i < packs.size(); i++) {
			PackageInfo p = packs.get(i);
			if (TextUtils.equals("com.tencent.mm", p.packageName)) {
				return true;
			}
		}
		return false;
