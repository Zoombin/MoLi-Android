...
public class Config {
	...
	// 新添加到购物车商品数
	public static int curShopCarNum = 0;
		
	//网络变化是否可用
	public static boolean connectivity = true;
	
	//客户端更服务器同步的超时时间(Tiket Login)
	...
	public static String getDeviceToken() {
		return UserInfoPreferences.getString(DEVICE_TOKEN, "");
	}

	public static void setDeviceToken(String deviceToken) {
		UserInfoPreferences.edit().putString(DEVICE_TOKEN, deviceToken).commit();
	}
	...
	public static boolean isShowNavigationBar(Context context){
		...
		int resourceId = resources.getIdentifier("navigation_bar_height", "dimen", "android");
		if (resourceId > 0) {
			return resources.getBoolean(rid); //获取导航栏是否显示true or false
		}
		return false;
	}
	...
	public static Typeface getTypeface() {
		return Typeface.createFromAsset(MoLiApplication.getContext().getAssets(),
				"fonts/zzgfyh.otf");
	}
}
