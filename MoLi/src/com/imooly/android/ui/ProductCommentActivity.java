...
import com.imooly.android.widget.AutoLoadListView;
...
public class ProductCommentActivity extends BaseActivity implements OnClickListener {
	...
	private AutoLoadListView store_comment_list;
	...
	private void initView() {
		...
		store_comment_list = (AutoLoadListView) findViewById(R.id.store_comment_list);
		store_comment_list.setOnLoadListener(loadmore);
	}
	...
	OnClickListener OptionMenuClick = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			curPage = 1;
			store_comment_list.setNoMoreData(false);
			getData(false);
		}
	};
		...
		Api.goodsCommentlist(self, goodsid, flag, curPage, 10, new NetCallBack<ServiceResult>() {
			...
			public void success(ServiceResult rspData) {
				...
				if (rsp.data != null) {
					...
					List<Comment> list = rsp.data.getCommentlist();
					if (addData) {
						if(list != null && list.size()>0){
							adapter.addData(list);
						}else{
							store_comment_list.setNoMoreData(true);
						}
					} else {
