...
public class OrderServiceDetail extends BaseActivity implements OnClickListener {
	...
	TextView tv_payback;
	LinearLayout ll_reVoucher;
	TextView tv_reVoucher;

	LinearLayout ll_level2;
	TextView tv_reason;
	LinearLayout ll_mark;
	TextView tv_mark;
	LinearLayout order_server_pic_layout;
	...
	private void initView() {
		...
		ll_reVoucher = (LinearLayout) findViewById(R.id.ll_reVoucher);
		ll_reVoucher.setVisibility(View.GONE);
		tv_reVoucher = (TextView) findViewById(R.id.tv_reVoucher);
		...
		order_server_pic_layout = (LinearLayout) findViewById(R.id.order_server_pic_layout);
		//order_server_pic_layout.setVisibility(View.GONE);
			...
			public void success(ServiceResult rspData) {
				...
				if (rsp.data != null) {
					...
					if (service != null) {
						...
						if ("return".equals(type)) {
							float payback = service.getRefundamount();
							float reVoucher = service.getRefundvoucher();
							
							ll_payback.setVisibility(View.VISIBLE);
							tv_payback.setText(payback+"");
							
							ll_reVoucher.setVisibility(View.VISIBLE);
							tv_reVoucher.setText(reVoucher+"");
						}else{
							ll_payback.setVisibility(View.GONE);
							ll_reVoucher.setVisibility(View.GONE);
						}
						...
						if (imgs != null && imgs.size() > 0) {
							order_server_pic_layout.setVisibility(View.VISIBLE);
					...
					if (address == null) {
						...
						if (!"return".equals(type)) {
							...
							zipcode.setText("邮编："+address.getPostcode());
