...
import com.imooly.android.entity.RspOrderProfile.Settle;
...
public class OrderDetailActivity extends BaseActivity implements OnClickListener {
	...
	private LinearLayout order_voucher_layout;//商品代金券
	private TextView order_voucher_value;
	private LinearLayout order_paid_layout;//商品代金券
	private TextView order_paid_value;
	...
	private void initView() {
		...
		order_voucher_layout = (LinearLayout) findViewById(R.id.order_voucher_layout);
		order_voucher_value = (TextView) findViewById(R.id.order_voucher_value);
		order_paid_layout = (LinearLayout) findViewById(R.id.order_paid_layout);
		order_paid_value = (TextView) findViewById(R.id.order_paid_value);
	...
	private void initData(RspOrderProfile rsp) {
		...
		if (addrEty != null) {
			...
			order_detail_zipcode_info.setText("邮编："+addrEty.getPostcode());
		...
		if (logisticEty != null) {
			...
			//imageLoader.displayImage(uri, order_detail_status_img);//状态图标
		...
		//付款总额
		Settle settle  = data.getSettle();
		order_voucher_layout.setVisibility(View.GONE);
		order_paid_layout.setVisibility(View.GONE);
		DecimalFormat fnum = new DecimalFormat("##0.00");
		if(settle != null){
			float voucher = settle.getVoucher();
			if(0 != voucher){
				order_voucher_layout.setVisibility(View.VISIBLE);
				order_voucher_value.setText("￥"+fnum.format(voucher));
			}
			float paid = settle.getTotalprice();
			if(0 != paid){
				order_paid_layout.setVisibility(View.VISIBLE);
				order_paid_value.setText("￥"+fnum.format(paid));
			}
		}
		...
		if (orderGdetys != null && orderGdetys.size() > 0) {
			for (int i = 0; i < orderGdetys.size(); i++) {
				final OrderGoodsEty gEty = orderGdetys.get(i);
				...
				TextView order_detail_descr_spec = (TextView) itemLayout.findViewById(R.id.order_detail_descr_spec);
				...
				order_detail_descr_price.setText("价格：" + fnum.format(gEty.getPrice()));
				order_detail_descr_num.setText("数量：" + gEty.getNum());
				order_detail_descr_spec.setText(gEty.getSpec());

				LinearLayout op_layout = (LinearLayout) itemLayout.findViewById(R.id.op_layout);
				HorizontalLinearLayout hlistview = (HorizontalLinearLayout) itemLayout.findViewById(R.id.hlistview);
				...
				if (opList != null && opList.size() > 0) {
					...
					//hlistview.setAdapter(opAdapter);
					//opAdapter.setOnclickLister(opHubClickListener);
					//opAdapter.setData(gEty, opList);
					hlistview.setOnclickLister(opHubClickListener);
					hlistview.setData(gEty, opList);
				} else {
				...
				itemLayout.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						intent.putExtra(ProductDetailActivity.EXTRA_GOODSID, gEty.getGoodsid());
						intent.setClass(self, ProductDetailActivity.class);
	...
	OpHubClickListener opHubClickListener = new OpHubClickListener() {
		...
		public void onClick(Object parentEntity, Object Entity) {
			...
			if ("OP016".equals(code)) {
				...
				OrderOperate.checkIsSettedTraderPassword(self, ety.getName(), ety.getPrice(), type, orderno, goodsid, tradeid, unique, oCallBack);
			}
