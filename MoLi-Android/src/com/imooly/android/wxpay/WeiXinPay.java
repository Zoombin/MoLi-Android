package com.imooly.android.wxpay;
...
public class WeiXinPay {
	...
	public WeiXinPay(Context context) {
		...
		mIp = Util.getLocalIpAddress();
	}
	
	public void pay(String body, String payNO, String price, String notifyUrl) {	
		this.mPayNo = payNO;
		this.mBody = body;
		this.mPrice = price;
	 ...
	 */
	private static final String PARTNER_KEY = "a219ee862f39adc9146268a7d20a436b";
     ...
     */
	private static final String APP_SECRET = "89f629c822b71cabfe761f96265b4f71"; // wxd930ea5d5a258f4f 对应的密钥
     ...
     * 注意：不能hardcode在客户端，建议genSign这个过程由服务器端完成
     */
	private static final String APP_KEY = "Jx51TawYD9iEBL0i4QC0RMc1fci5C1V435ZtyRVCmERHpJyRUnbWY8QGuhEExFhxaD3joCAHbJnbXoeozNEqixlrw6Tgol9uJ9uM4HRcwOnWFtI1uOE3h5l4nlH1d9HJ"; // wxd930ea5d5a258f4f 对应的支付密钥
	...
	private class GetAccessTokenTask extends AsyncTask<Void, Void, GetAccessTokenResult> {
		...
		protected void onPostExecute(GetAccessTokenResult result) {
			...
			if (result.localRetCode == LocalRetCode.ERR_OK) {
				...
				Toast.makeText(mContext, "获取access token失败" + result.localRetCode.name(), Toast.LENGTH_LONG).show();
			}
		}
	...
	private class GetPrepayIdTask extends AsyncTask<Void, Void, GetPrepayIdResult> {
		...
		protected void onPostExecute(GetPrepayIdResult result) {
			...
			if (result.localRetCode == LocalRetCode.ERR_OK) {
				Toast.makeText(mContext, "获取预支付订单成功", Toast.LENGTH_LONG).show();
				sendPayReq(result);
			} else {
				Toast.makeText(mContext,  "获取预支付订单成功"  + result.localRetCode.name(), Toast.LENGTH_LONG).show();
			}
		}
...
//	}
	...
	private String genProductArgs() {
		...
		try {
			...
			packageParams.add(new BasicNameValuePair("out_trade_no", mPayNo)); // 订单号
			packageParams.add(new BasicNameValuePair("partner", Constants.PARTNER_ID));
			packageParams.add(new BasicNameValuePair("spbill_create_ip", mIp));
			packageParams.add(new BasicNameValuePair("total_fee", mPrice));  // 单位为分
