...
public class OrderServiceStatusAdapter extends BaseAdapter implements OnClickListener {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			tag.hlistview = (HorizontalListView) convertView.findViewById(R.id.hlistview);
		...
		if (ety != null) {
			...
			tag.item_price.setText("价格：" + "￥" + ety.getPrice());
	...
	OpHubClickListener opHubClickListener = new OpHubClickListener() {
		...
		public void onClick(Object parentEntity, Object Entity) {
			...
			if ("OP013".equals(code)) {
				// OP013 取消售后
				OrderOperate.servicecancel(context, orderno, goodsid, tradeid, type, null);
			}
			...
			if ("OP015".equals(code)) {
				// OP015 延迟收货
				// 这里的type是 换货/正常（change/nomal）
				OrderOperate.takedelay(context, "change", orderno, goodsid, tradeid, unique, null);
			}
			if ("OP016".equals(code)) {
				// OP016 确认收货
				// 这里的type是 换货/正常（change/nomal）
				// OrderOperate.takeDelivery(context, orderno, callBack);
			...
			if ("OP017".equals(code)) {
				// OP017 人工服务
				OrderOperate.servicemanual(context, orderno, goodsid, tradeid, type, null);
			}
		}
	};
}
